...
title: 'Release [FORTISHIELD VERSION] - Release Candidate [RC VERSION] - WPK upgrade tests'
labels: 'WPK, core'
assignees: ''
...
Fortishield versions to test (Upgrade to the current agent version):
### Linux
...
## Windows:
### UPGRADE FAIL
- [ ] Fortishield-Agent folder tree:  No files are lost after the rollback. The logs of the failed upgrade (`ossec.log`) are kept.
- [ ] After the rollback the agent connects to the manager
- [ ] After reboot, the Fortishield-Agent starts and connects to the manager.
- [ ] The correct Fortishield-Agent version is shown in the list of Windows' `programs and features`.
- [ ] A new version of Fortishield-Agent can be manually installed via MSI after the rollback process.

### UPGRADE OK

- [ ] Message `Upgrade finished successfully.` is shown in `upgrade.log` file.
- [ ] Fortishield service is started and the agent is connected to the manager.
