...
function install_wazuh(){
  ...
  eval "echo \"$1\" > ${FORTISHIELD_MACOS_AGENT_DEPLOYMENT_VARS} && installer -pkg wazuh-agent-${VERSION}-0.commit${SHA}.pkg -target / > /dev/null 2>&1"
...
install_wazuh "FORTISHIELD_REGISTRATION_CA='/Library/Ossec/etc/testsslmanager.cert'"
test "FORTISHIELD_REGISTRATION_CA"
remove_wazuh

install_wazuh "FORTISHIELD_REGISTRATION_CERTIFICATE='/Library/Ossec/etc/testsslmanager.cert'"
test "FORTISHIELD_REGISTRATION_CERTIFICATE"
remove_wazuh

install_wazuh "FORTISHIELD_REGISTRATION_KEY='/Library/Ossec/etc/testsslmanager.key'"
