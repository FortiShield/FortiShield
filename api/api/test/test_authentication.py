...
security_conf = FortishieldResult({
...
async def test_decode_token(mock_raise_if_exc, mock_distribute_function, mock_dapi, mock_generate_keypair,
    ...
    mock_raise_if_exc.side_effect = [FortishieldResult({'valid': True, 'policies': {'value': 'test'}}),
                                     FortishieldResult(security_conf)]
            ...
            with patch('fortishield.core.cluster.dapi.dapi.DistributedAPI.__init__', return_value=None):
                with patch('fortishield.core.cluster.dapi.dapi.DistributedAPI.distribute_function'):
                    with patch('api.authentication.raise_if_exc') as mock_raise_if_exc:
                        ...
                        with pytest.raises(Unauthorized):
                            mock_raise_if_exc.side_effect = [FortishieldResult({'valid': False})]
                            authentication.decode_token(token='test_token')

                        with pytest.raises(Unauthorized):
                            mock_raise_if_exc.side_effect = [
                                FortishieldResult({'valid': True, 'policies': {'value': 'test'}}),
                                FortishieldResult({'auth_token_exp_timeout': 900,
                                             'rbac_mode': 'white'})]
                            authentication.decode_token(token='test_token')
