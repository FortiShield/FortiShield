...
@pytest.mark.parametrize('exception_type, code, extra_fields, returned_code, returned_exception', [
    ...
    (FortishieldError, 1000, ['remediation', 'code'], 400, ProblemException),
    (FortishieldPermissionError, 4000, ['remediation', 'code'], 403, ProblemException),
    (FortishieldResourceNotFound, 1710, ['remediation', 'code'], 404, ProblemException),
    (FortishieldInternalError, 1000, ['remediation', 'code'], 500, ProblemException)
])
...
@pytest.mark.parametrize('obj, code', [
    ((FortishieldError(6001), ['value0', 'value1']), 429),
    ((FortishieldInternalError(1000), ['value0', 'value1']), None),
    ((FortishieldPermissionError(4000), ['value0', 'value1']), None),
    ((FortishieldResourceNotFound(1710), ['value0', 'value1']), None)
])
...
def test_only_master_endpoint(mock_exc):
    ...
    with patch('api.util.running_in_master_node', return_value=False):
        func_()
        mock_exc.assert_called_once_with(FortishieldResourceNotFound(902))
