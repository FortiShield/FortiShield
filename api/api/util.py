...
def to_relative_path(full_path: str) -> str:
    """Return a relative path from Fortishield base directory.
    ...
    -------
    str
        Relative path from Fortishield base directory.
    """
    return os.path.relpath(full_path, common.FORTISHIELD_PATH)
...
def _create_problem(exc: Exception, code: int = None):
    ...
    ----------
    exc : Exception
        If `exc` is an instance of `FortishieldException` it will be casted into a ProblemException, otherwise it will be
        raised.
    code : int
...
def only_master_endpoint(func):
    ...
    def wrapper(*args, **kwargs):
        if not running_in_master_node():
            raise_if_exc(exception.FortishieldResourceNotFound(902))
        else:
