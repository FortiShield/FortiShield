...
FORTISHIELD_COMPONENT_CONFIGURATION_MAPPING = MappingProxyType(
    {
...
def is_safe_path(path: str, basedir: str = common.FORTISHIELD_PATH, relative: bool = True) -> bool:
    ...
    path : str
        Path to be checked.
    basedir : str
        Fortishield installation directory.
    relative : bool
...
def check_component_configuration_pair(component: str, configuration: str) -> FortishieldError:
    """

    Parameters
    ----------
    component : str
        Fortishield component name.
    configuration : str
    ...
    -------
    FortishieldError
        It can either return a `FortishieldError` or `None`, depending on the given component and configuration. The exception
        is returned and not raised because we use the object to create a problem on API level.
    """
    if configuration not in FORTISHIELD_COMPONENT_CONFIGURATION_MAPPING[component]:
        return FortishieldError(1128, extra_message=f"Valid configuration values for '{component}': "
                                              f"{FORTISHIELD_COMPONENT_CONFIGURATION_MAPPING[component]}")
