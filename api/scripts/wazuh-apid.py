...
def start():
    """Run the Fortishield API.

    If another Fortishield API is running, this function fails.
    ...
    app.add_api('spec.yaml',
                arguments={'title': 'Fortishield API',
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"Error when trying to start the Fortishield API. {e}")
        sys.exit(1)
    ...
    if 'use_only_authd' in api_conf:
        ...
        logger.warning("'use_only_authd' option was deprecated on v4.3.0. Fortishield Authd will always be used")
    ...
    if api_conf['https']['enabled']:
        try:
            ...
            if not os.path.exists(api_conf['https']['key']) or not os.path.exists(api_conf['https']['cert']):
                ...
                logger.info(
                    f"Generated private key file in FORTISHIELD_PATH/{to_relative_path(api_conf['https']['key'])}")
                configuration.generate_self_signed_certificate(private_key, api_conf['https']['cert'])
                logger.info(
                    f"Generated certificate file in FORTISHIELD_PATH/{to_relative_path(api_conf['https']['cert'])}")
            ...
            if exc.errno == 22:
                ...
                msg = f'Fortishield API SSL ERROR. Please, ensure if path to certificates is correct in the configuration ' \
                      f'file FORTISHIELD_PATH/{to_relative_path(CONFIG_FILE_PATH)}'
                print(msg)
                logger.error(msg)
                raise exc
    ...
    try:
        ...
        print(f'Internal error when trying to start the Fortishield API. {e}')
        sys.exit(1)
    finally:
