...
class DistributedAPI:
    ...
    async def distribute_function(self) -> [Dict, exception.FortishieldException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or FortishieldException
            Dictionary with API response or FortishieldException in case of error.
        """
        try:
            ...
            return response if isinstance(response, (wresults.AbstractFortishieldResult, exception.FortishieldException)) \
                else wresults.FortishieldResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.FortishieldInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.FortishieldInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.FortishieldClusterError))
            return e
        except exception.FortishieldError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
    ...
    def check_fortishield_status(self):
        ...
        if not_ready_daemons:
            ...
            raise exception.FortishieldError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.FortishieldInternalError(3021)
                except OperationalError as exc:
                    raise exception.FortishieldInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.FortishieldInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.FortishieldInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.FortishieldInternalError(900)
            ...
            # FortishieldClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913} and not isinstance(e,
                                                                                           exception.FortishieldClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.FortishieldJSONEncoder)
        except (exception.FortishieldError, exception.FortishieldResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.FortishieldJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.FortishieldInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.FortishieldException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.FortishieldException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of FortishieldInternalError
        if isinstance(e, exception.FortishieldInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('FORTISHIELD_HOME', os.path.relpath(h.baseFilename, start=common.FORTISHIELD_PATH))
    ...
    async def forward_request(self) -> [wresults.AbstractFortishieldResult, exception.FortishieldException]:
        ...
        Returns
        -------
        wresults.AbstractFortishieldResult or exception.FortishieldException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractFortishieldResult, exception.FortishieldException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a FortishieldError exception
                if isinstance(result, FortishieldClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = FortishieldError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractFortishieldResult, exception.FortishieldException)) \
                else wresults.FortishieldResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsFortishieldResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractFortishieldResult):
        ...
        # It might be a FortishieldError after reducing
        if isinstance(response, wresults.AffectedItemsFortishieldResult):
...
class FortishieldRequestQueue:
    """Represents a queue of Fortishield requests"""
...
class APIRequestQueue(FortishieldRequestQueue):
    """
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.FortishieldJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(FortishieldRequestQueue):
    """
