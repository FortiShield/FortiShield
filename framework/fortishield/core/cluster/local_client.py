    ...
    async def start(self):
        ...
        try:
            self.transport, self.protocol = await loop.create_unix_connection(
                                             ...
                                             path=os.path.join(common.FORTISHIELD_PATH, 'queue', 'cluster', 'c-internal.sock'))
        except (ConnectionRefusedError, FileNotFoundError):
            raise exception.FortishieldInternalError(3012)
        except MemoryError:
            raise exception.FortishieldInternalError(1119)
        except Exception as e:
            raise exception.FortishieldInternalError(3009, str(e))
        ...
        while True:
            ...
            try:
                ...
                if min_timeout < self.cluster_items['intervals']['worker']['keep_alive']:
                    raise exception.FortishieldInternalError(3020)
                else:
                    try:
