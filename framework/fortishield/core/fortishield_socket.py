...
class FortishieldSocket:
    ...
    def _connect(self):
        try:
            ...
            raise FortishieldInternalError(1013, extra_message=os.path.basename(self.path))
        except ConnectionRefusedError:
            raise FortishieldInternalError(1121, extra_message=f"Socket '{os.path.basename(self.path)}' cannot receive "
                                                         "connections")
        except Exception as e:
            raise FortishieldException(1013, str(e))
    ...
    def send(self, msg_bytes, header_format="<I"):
        if not isinstance(msg_bytes, bytes):
            raise FortishieldException(1105, "Type must be bytes")

        try:
            sent = self.s.send(pack(header_format, len(msg_bytes)) + msg_bytes)
            if sent == 0:
                raise FortishieldException(1014, "Number of sent bytes is 0")
            return sent
        except Exception as e:
            raise FortishieldException(1014, str(e))
...
class FortishieldSocketJSON(FortishieldSocket):
    MAX_SIZE = 65536

    def __init__(self, path):
        FortishieldSocket.__init__(self, path)

    def send(self, msg, header_format="<I"):
        return FortishieldSocket.send(self, msg_bytes=dumps(msg).encode(), header_format=header_format)

    def receive(self, header_format="<I", header_size=4, raw=False):
        response = loads(FortishieldSocket.receive(self, header_format=header_format, header_size=header_size).decode())
        if not raw:
            if 'error' in response.keys():
                if response['error'] != 0:
                    raise FortishieldException(response['error'], response['message'], cmd_error=True)
            return response['data']
        else:
            return response


class FortishieldAsyncProtocol(asyncio.Protocol):
    """Fortishield implementation of asyncio.Protocol class."""
...
class FortishieldAsyncSocket:
    ...
    async def connect(self, path_to_socket: str):
        ...
        try:
            ...
            raise FortishieldException(1013, str(exc)) from exc
    ...
    async def send(self, msg_bytes: bytes, header_format: str = "<I"):
        ...
        Raises
        ------
        FortishieldException(1014)

        """
        if not isinstance(msg_bytes, bytes):
            raise FortishieldException(1014, "Type must be bytes")
        ...
        try:
            ...
            raise FortishieldException(1014, "Socket connection was closed") from exc
    ...
    async def receive(self, header_format: str ="<I", header_size: int = 4) -> bytes:
        ...
        try:
            ...
            raise FortishieldException(1014, str(exc)) from exc


class FortishieldAsyncSocketJSON(FortishieldAsyncSocket):
...
async def fortishield_sendasync(daemon_name: str, message: str = None) -> dict:
    ...
    dict
        Data received.
    """
    sock = FortishieldAsyncSocketJSON()
...
async def fortishield_sendsync(daemon_name: str = None, message: str = None) -> dict:
    ...
    """
    try:
        sock = FortishieldSocket(daemons[daemon_name]['path'])
        ...
        raise FortishieldInternalError(1014, extra_message=e)
