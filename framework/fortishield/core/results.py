...
class AbstractFortishieldResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.FortishieldInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractFortishieldResult):
            self.dikt = dct.dikt
        else:
            raise wexception.FortishieldInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractFortishieldResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractFortishieldResult objects. When merged with a FortishieldException, the result is
        always a FortishieldException

        Parameters
        ----------
        other : AbstractFortishieldResult or wexception.FortishieldException

        Raises
        ------
        FortishieldInternalError(1000)
            Fortishield internal error when two object could not be merged.

        Returns
        -------
        AbstractFortishieldResult or wexception.FortishieldException
            Resultant object.
        """
        if isinstance(other, wexception.FortishieldException):
            return other
        elif not isinstance(other, (dict, AbstractFortishieldResult)):
            raise wexception.FortishieldInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractFortishieldResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in FortishieldResult subclasses.
        ...
        Returns
        -------
        AbstractFortishieldResult
            Filtered AbstractFortishieldResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in FortishieldResult subclasses.
        ...
        -------
        AbstractFortishieldResult
            Sorted AbstractFortishieldResult
        """
...
class FortishieldResult(AbstractFortishieldResult):
    """
...
class AffectedItemsFortishieldResult(AbstractFortishieldResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.FortishieldException = None):
        ...
        error : wexception.FortishieldException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsFortishieldResult
        ...
        wexception.FortishieldInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsFortishieldResult):
            raise wexception.FortishieldInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsFortishieldResult with self.

        Parameters
        ----------
        other : AffectedItemsFortishieldResult
            AffectedItemsFortishieldResult object to merge.
        ...
        if isinstance(other, wexception.FortishieldError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.FortishieldException):
            return result
        elif not isinstance(other, AffectedItemsFortishieldResult):
            raise wexception.FortishieldInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsFortishieldResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractFortishieldResult
            Instance of AbstractFortishieldResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsFortishieldResult object.

        Returns
        -------
        dict
            Return AffectedItemsFortishieldResult as a dictionary with fields data, message and error.
        """
