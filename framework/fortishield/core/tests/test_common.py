...
from fortishield.core.common import find_fortishield_path, fortishield_uid, fortishield_gid, context_cached, reset_context_cache, \
    get_context_cache


@pytest.mark.parametrize('fake_path, expected', [
    ('/var/ossec/framework/python/lib/python3.7/site-packages/fortishield-3.10.0-py3.7.egg/fortishield', '/var/ossec'),
    ('/my/custom/path/framework/python/lib/python3.7/site-packages/fortishield-3.10.0-py3.7.egg/fortishield', '/my/custom/path'),
    ('/my/fake/path', '')
])
def test_find_fortishield_path(fake_path, expected):
    with patch('fortishield.core.common.__file__', new=fake_path):
        assert (find_fortishield_path.__wrapped__() == expected)


def test_find_fortishield_path_relative_path():
    with patch('os.path.abspath', return_value='~/framework'):
        assert (find_fortishield_path.__wrapped__() == '~')


def test_fortishield_uid():
    with patch('fortishield.core.common.getpwnam', return_value=getpwnam("root")):
        fortishield_uid()


def test_fortishield_gid():
    with patch('fortishield.core.common.getgrnam', return_value=getgrnam("root")):
        fortishield_gid()
...
@patch('fortishield.core.logtest.create_fortishield_socket_message', side_effect=SystemExit)
...
async def test_origin_module_context_var_api(mock_check_fortishield_status, mock_create_socket_msg):
    ...
    # side_effect used to avoid mocking the rest of functions
    with pytest.raises(SystemExit):
        d = dapi.DistributedAPI(f=logtest.run_logtest, logger=logging.getLogger('fortishield'), is_async=True)
