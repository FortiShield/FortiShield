...
def test_totals_ko_():
    ...
    with patch("builtins.open", mock_open(read_data='15-571-3-2\n15--107--1483')):
        with pytest.raises(FortishieldInternalError, match=".* 1309 .*"):
            stats.totals_(date(1996, 8, 13))
...
@pytest.mark.parametrize('agent, daemon, expected_value', [
    (0, 'logcollector', os.path.join(common.FORTISHIELD_PATH, "queue", "sockets", 'logcollector')),
...
async def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(FortishieldInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('fortishield.core.fortishield_socket.FortishieldSocket.__init__', return_value=None) as mock_socket:
        ...
        if agent_id == '000':
            mock_socket.assert_called_once_with(os.path.join(common.FORTISHIELD_PATH, "queue", "sockets", "logcollector"))
            mock_send.assert_called_once_with(b'getstate')
        else:
            mock_socket.assert_called_once_with(os.path.join(common.FORTISHIELD_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket_ko():
    ...
    with pytest.raises(FortishieldError, match=r'\b1307\b'):
        stats.get_daemons_stats_from_socket(None, None)

    with pytest.raises(FortishieldError, match=r'\b1310\b'):
