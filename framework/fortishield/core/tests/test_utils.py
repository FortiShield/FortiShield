...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.FortishieldException, match=".* 1723 .*"):
...
@pytest.mark.parametrize('version1, version2', [
    ...
    ('Fortishield v3.6.1', 'Fortishield v3.6.3'),
    ('Fortishield v3.7.2', 'Fortishield v3.8.0'),
    ('Fortishield v3.8.0', 'Fortishield v3.8.1'),
    ('Fortishield v3.9.0', 'Fortishield v3.9.2'),
    ('Fortishield v3.9.10', 'Fortishield v3.9.14'),
    ('Fortishield v3.10.1', 'Fortishield v3.10.10'),
    ('Fortishield v4.10.10', 'Fortishield v4.11.0'),
    ('Fortishield v5.1.15', 'Fortishield v5.2.0'),
    ('v3.6.0', 'v3.6.1'),
    ('v3.9.1', 'v3.9.2'),
    ('v4.0.0', 'v4.0.1'),
    ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test FortishieldVersion class."""
    current_version = utils.FortishieldVersion(version1)
    new_version = utils.FortishieldVersion(version2)
...
@pytest.mark.parametrize('version1, version2', [
    ('v3.6.0', 'v.3.6.1'),
    ('Fortishield v4', 'Fortishield v5'),
    ('Fortishield v3.9', 'Fortishield v3.10'),
    ('ABC v3.10.1', 'ABC v3.10.12'),
    ('Fortishieldv3.9.0', 'Fortishieldv3.9.2'),
    ('3.9', '3.10'),
    ('3.9.0', '3.10'),
    ('3.10', '4.2'),
    ('3', '3.9.1')
...
def test_version_ko(version1, version2):
    """Test FortishieldVersion class."""
    try:
        utils.FortishieldVersion(version1)
        utils.FortishieldVersion(version2)
    except ValueError:
        return


@pytest.mark.parametrize('version1, version2', [
    ('Fortishield v3.10.10', 'Fortishield v3.10.10'),
    ('Fortishield v5.1.15', 'Fortishield v5.1.15'),
    ('v3.6.0', 'v3.6.0'),
    ('v3.9.2', 'v3.9.2')
])
...
def test_FortishieldVersion_to_array():
    """Test FortishieldVersion.to_array function."""
    version = utils.FortishieldVersion('Fortishield v3.10.0-alpha4')

    assert isinstance(version.to_array(), list)


def test_FortishieldVersion__str__():
    """Test FortishieldVersion.__str__ function."""
    version = utils.FortishieldVersion('Fortishield v3.10.0-alpha4')
...
@pytest.mark.parametrize('version1, version2', [
    ('Fortishield v3.5.2', 'Fortishield v4.0.0'),
    ('Fortishield v3.10.0-alpha', 'Fortishield v3.10.0'),
    ('Fortishield v3.10.0-alpha4', 'Fortishield v3.10.0-beta4'),
    ('Fortishield v3.10.0-alpha3', 'Fortishield v3.10.0-alpha4'),
])
def test_FortishieldVersion__ge__(version1, version2):
    """Test FortishieldVersion.__ge__ function."""
    current_version = utils.FortishieldVersion(version1)
    new_version = utils.FortishieldVersion(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.FortishieldException, match=".* 1411 .*"):
...
def test_FortishieldDBQuery_protected_clean_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=500, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=1),
...
def test_FortishieldDBQuery_protected_add_limit_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=limit, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=1),
...
def test_FortishieldDBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=1), count=5,
                               get_data=None)
...
def test_FortishieldDBQuery_protected_add_sort_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
    ...
    """Test FortishieldDBQuery._add_sort_to_query function."""
    fields = {'1': 'one', '2': 'two', '3': 'three', '4': 'four'}
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=sort,
                               search=None, select=None, filters=None,
                               fields=fields,
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=1),
...
def test_FortishieldDBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test FortishieldDBQuery._add_search_to_query function."""
...
def test_FortishieldDBQuery_protected_parse_select_filter(mock_socket_conn, mock_glob, mock_conn_db, mock_exists,
    ...
    """Test FortishieldDBQuery._parse_select_filter function."""
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
def test_FortishieldDBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_glob,
                                                     mock_exists, filter_):
    """Test FortishieldDBQuery._parse_legacy_filters function."""
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=filter_,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=0),
                               count=5, get_data=None)
...
def test_FortishieldDBQuery_protected_process_filter(mock_date, mock_status, mock_socket_conn, mock_conn_db,
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               ...
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=0), count=5,
...
def test_FortishieldDBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn,
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               ...
                               backend=utils.FortishieldDBBackend(agent_id=0), distinct=True,
...
def test_FortishieldDBQuery_protected_add_filters_to_query_final_query(mock_conn_db, mock_file_exists,
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None, search=None, select=None,
                               fields={'id': 'id', 'status': 'status', 'group': 'group'},
                               default_sort_field=None, query='', backend=utils.FortishieldDBBackend(agent_id=0),
                               count=5, get_data=None)
...
def test_FortishieldDBQuery_protected_get_total_items_mitre(mock_socket_conn, mock_conn_db, mock_glob,
    ...
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               ...
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.FortishieldDBBackend(agent_id=0, query_format='mitre'), count=5,
                               get_data=None, date_fields=['date1', 'date2'])
...
def test_FortishieldDBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.FortishieldDBQuery._format_data_into_dictionary."""
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
...
def test_FortishieldDBQuery_protected_filter_date(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
    ...
    """Test utils.FortishieldDBQuery._filter_date function."""
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
def test_FortishieldDBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test utils.FortishieldDBQuery._default_count_query function."""
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
def test_FortishieldDBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            value):
    """Test utils.FortishieldDBQuery._pass_filter function."""
    query = utils.FortishieldDBQuery(offset=0, limit=1, table='agent', sort=None,
...
def test_FortishieldDBQueryDistinct_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    """Test utils.FortishieldDBQueryDistinct._default_query function."""
    query = utils.FortishieldDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
def test_FortishieldDBQueryDistinct_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.FortishieldDBQueryDistinct._default_count_query function."""
    query = utils.FortishieldDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
def test_FortishieldDBQueryDistinct_protected_add_select_to_query(mock_add, mock_socket_conn, mock_conn_db,
    ...
    if len(select) > 1:
        with pytest.raises(exception.FortishieldException, match=".* 1410 .*"):
            query._add_select_to_query()
    else:
        query._add_select_to_query()
...
def test_FortishieldDBQueryDistinct_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db,
                                                                    mock_glob, mock_exists):
    """Test utils.FortishieldDBQueryDistinct._format_data_into_dictionary function."""
    query = utils.FortishieldDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'fields': ['name']},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True,
...
def test_FortishieldDBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    ...
    utils.FortishieldDBQueryGroupBy(filter_fields=None, offset=0, limit=1, table='agent',
                              ...
                              backend=utils.FortishieldDBBackend(agent_id=0), get_data=None,
...
def test_FortishieldDBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db,
    ...
    query = utils.FortishieldDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.FortishieldError, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.FortishieldError as e:
        assert e.code == 1724
...
def test_to_relative_path():
    ...
    path = 'etc/ossec.conf'
    assert utils.to_relative_path(os.path.join(FORTISHIELD_PATH, path)) == path
...
def test_agents_allow_higher_versions(new_conf, agents_conf):
    ...
    with patch('fortishield.core.utils.configuration.api_conf', new=api_conf):
        if agents_conf['allow_higher_versions']['allow'] or new_conf.find('no') != -1:
            ...
            with pytest.raises(exception.FortishieldError, match=".* 1129 .*"):
                utils.check_agents_allow_higher_versions(new_conf)
...
def test_upload_file(mock_uid, mock_gid, 
    ...
    with patch('fortishield.core.utils.open', mko):
        with patch('fortishield.core.utils.safe_move') as mock_safe_move:
            ...
            file_path = os.path.join(FORTISHIELD_PATH, filename)
...
@pytest.mark.parametrize(
    'sm_side_effect, w_side_effect, upload_error', [
        ...
        (None, True, (utils.FortishieldInternalError, 1005)),
        # safe_move raises an Error()
        (Error(), False, (utils.FortishieldInternalError, 1016)),
