...
def test_fortishield_logger_setup_logger(mock_time_handler, mock_size_handler, mock_add_handler, mock_add_level_name,
    ...
    with patch('os.path.join', return_value=tmp_dir.name):
        w_logger = wlogging.FortishieldLogger(foreground_mode=True, log_path=tmp_dir.name,
...
@patch.object(wlogging.FortishieldLogger, 'log_path', create=True, new_callable=PropertyMock)
@patch.object(wlogging.FortishieldLogger, 'tag', create=True, new_callable=PropertyMock)
@patch.object(wlogging.FortishieldLogger, 'logger', create=True, new_callable=PropertyMock)
@patch.object(wlogging.FortishieldLogger, 'foreground_mode', create=True, new_callable=PropertyMock)
@patch.object(wlogging.FortishieldLogger, 'debug_level', create=True, new_callable=PropertyMock)
@patch.object(wlogging.FortishieldLogger, 'logger_name', create=True, new_callable=PropertyMock)
...
def test_fortishield_logger__init__(mock_lformatter, mock_max_size, mock_formatter, mock_logger_name, mock_debug_level,
    ...
    with patch('os.path.join'):
        wlogging.FortishieldLogger(foreground_mode=mock_foreground_mode, log_path=mock_log_path, tag=mock_tag,
...
def test_fortishield_logger_getattr(mock_time_handler, mock_size_handler, attribute, expected_exception, expected_value):
    ...
    # To bypass the checking of the existence of a valid Fortishield install
    with patch('os.path.join'):
        w_logger = wlogging.FortishieldLogger(foreground_mode=True, log_path=tmp_dir.name, tag='%(test)s %(test)s: %(test)s',
