    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (FORTISHIELD_USER_ID, FORTISHIELD_WUI_USER_ID):
        ...
        if from_id == FORTISHIELD_USER_ID and to_id == FORTISHIELD_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set Fortishield ownership and permissions.
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=FORTISHIELD_USER_ID,
                                        to_id=FORTISHIELD_WUI_USER_ID)
        ...
        logger.error("Error retrieving the current Fortishield RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
