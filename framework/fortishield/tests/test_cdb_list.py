...
def test_get_lists_sort():
    ...
    assert isinstance(result_a, AffectedItemsFortishieldResult)
    assert isinstance(result_b, AffectedItemsFortishieldResult)
...
def test_get_list_file(filename, raw, expected_result, total_failed_items):
    ...
    with patch('fortishield.cdb_list.get_filenames_paths', return_value=[os.path.join(DATA_PATH, 'test_lists_2')]):
        ...
        if raw:
            ...
            isinstance(result, AffectedItemsFortishieldResult)
