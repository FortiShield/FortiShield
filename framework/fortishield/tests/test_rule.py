...
def test_get_rules_files_status_include(mock_ossec, status, func):
    ...
    if status == 'random':
        ...
        with pytest.raises(FortishieldError, match='.* 1202 .*'):
            func(status=status)
    else:
...
def test_failed_get_rules_file(mock_config):
    ...
    """
    with pytest.raises(FortishieldError, match=".* 1200 .*"):
        rule.get_rules_files()
...
def test_failed_get_rules():
    ...
    with pytest.raises(FortishieldError, match=".* 1203 .*"):
...
def test_get_rule_file(filename, raw, relative_dirname, contains):
    ...
    with patch('fortishield.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        if raw:
            ...
            assert isinstance(d_files, AffectedItemsFortishieldResult)
