...
@pytest.mark.parametrize('api_response, total_affected_items', [
    ({'version': 'Fortishield v4.2.1', 'id': '002', 'name': 'test'}, 1),
    ({}, 0)
])
...
async def test_main_ko(capfd):
    ...
    with patch('scripts.agent_upgrade.list_outdated', side_effect=FortishieldError(1000)):
        with pytest.raises(FortishieldError, match='.* 1000 .*'):
            await agent_upgrade.main()
        out, err = capfd.readouterr()
        assert out == 'Error 1000: Fortishield Internal Error\n'
