...
async def test_print_agents(local_client_mock, get_agents_mock, print_table_mock, itemgetter_mock, map_mock):
    ...
    filter_node = 'fortishield_worker'
...
@patch('scripts.cluster_control.local_client.LocalClient', return_value='LocalClient return value')
@patch('scripts.cluster_control.control.get_nodes', return_value={'items': [{'name': 'fortishield_worker'}]})
@patch('scripts.cluster_control.control.get_health',
       return_value={'n_connected_nodes': '1',
                     'nodes': {'fortishield_worker2': {
...
async def test_print_health(get_health_mock, get_nodes_mock, local_client_mock, get_utc_strptime_mock, print_mock):
    ...
    more = True
    worker_status = get_health_mock.return_value['nodes']['fortishield_worker2']['status']
    worker_info = get_health_mock.return_value['nodes']['fortishield_worker2']['info']
    get_utc_strptime_mock.side_effect = seconds_mock

    # Test cases 1 and 2
    for filter_node in ['fortishield_worker', None]:
        # Reset mocks
        print_mock.reset_mock()
        ...
        print_mock.assert_has_calls([call(f"Cluster name: {config['name']}\n\n"
                                     ...
                                     call(f"\n    fortishield_worker2 ({worker_info['ip']})\n        "
    ...
    print_mock.assert_called_once_with(f"Cluster name: {config['name']}\n\nLast completed synchronization for connected"
                                       ...
                                       f"fortishield_worker2 "
                                       f"({get_health_mock.return_value['nodes']['fortishield_worker2']['info']['ip']}): "
...
@patch('fortishield.core.cluster.cluster.check_cluster_config')
@patch('fortishield.core.cluster.utils.read_config', return_value='')
@patch('fortishield.core.cluster.utils.get_cluster_status', return_value={'enabled': 'no', 'running': 'yes'})
