    ...
    for name in dir_content:
        ...
        if (path.isfile(new_absolute_path)) and ('.cdb' not in name) and ('~' not in name) and not pattern.search(name):
            if only_names:
                ...
                items = get_list_from_file(path.join(common.FORTISHIELD_PATH, new_relative_path))
...
def split_key_value_with_quotes(line: str, file_path: str = '/CDB_LISTS_PATH') -> tuple:
    ...
    if line.count('"') == 4:
        ...
        if first_quote != 0 or line[second_quote: third_quote + 1] != '":"' or fourth_quote != len(
                line) - 1:
            raise FortishieldError(1800, extra_message={'path': path.join('FORTISHIELD_HOME', file_path)})
...
def get_list_from_file(path: str, raw: bool = False) -> Union[dict, str]:
    ...
    FortishieldError(1802)
        CDB list file not found.
    FortishieldError(1803)
        Error reading list file (permissions).
    FortishieldError(1804)
    ...
    try:
        ...
        if e.errno == 2:
            raise FortishieldError(1802)
        elif e.errno == 13:
            raise FortishieldError(1803)
        elif e.errno == 21:
            raise FortishieldError(1804, extra_message="{0} {1}".format(path, "is a directory"))
        else:
            raise e
    except ValueError:
        raise FortishieldError(1800, extra_message={'path': path})
...
def create_list_file(full_path: str, content: str, permissions: int = 0o660) -> str:
    ...
    try:
        ...
        raise FortishieldError(1806)

    return full_path


def delete_list(rel_path: str):
    """Delete a Fortishield CDB list file.
    ...
    try:
        remove(path.join(common.FORTISHIELD_PATH, rel_path + common.COMPILED_LISTS_EXTENSION))
