...
def check_cluster_config(config):
    ...
    if len(config['key']) == 0:
        raise FortishieldError(3004, 'Unspecified key')

    elif not iv.check_name(config['key']) or not iv.check_length(config['key'], 32, eq):
        raise FortishieldError(3004, 'Key must be 32 characters long and only have alphanumeric characters')

    elif config['node_type'] != 'master' and config['node_type'] != 'worker':
        raise FortishieldError(3004, f'Invalid node type {config["node_type"]}. Correct values are master and worker')

    elif not isinstance(config['port'], int):
        raise FortishieldError(3004, "Port has to be an integer.")

    elif not 1024 < config['port'] < 65535:
        raise FortishieldError(3004, "Port must be higher than 1024 and lower than 65535.")
    ...
    if len(invalid_elements) != 0:
        raise FortishieldError(3004, f"Invalid elements in node fields: {', '.join(invalid_elements)}.")
...
def walk_dir(dirname, recursive, files, excluded_files, excluded_extensions, get_cluster_item_key, previous_status=None,
    ...
    full_dirname = path.join(common.FORTISHIELD_PATH, dirname)
    ...
    try:
                ...
                for file_ in files_:
                    ...
                    try:
                        ...
                        if files == ['all'] or file_ in files:
                            relative_file_path = path.join(path.relpath(root_, common.FORTISHIELD_PATH), file_)
        ...
        raise FortishieldInternalError(3015, e)
...
def compress_files(name, list_path, cluster_control_json=None, max_zip_size=None):
    ...
    zip_file_path = path.join(common.FORTISHIELD_PATH, 'queue', 'cluster', name,
    ...
    with open(zip_file_path, 'ab') as wf:
        for file in list_path:
            ...
            try:
                with open(path.join(common.FORTISHIELD_PATH, file), 'rb') as rf:
                    new_file = rf.read()
                    if len(new_file) > max_zip_size:
                        result_logs['warning'][file].append(f'File too large to be synced: '
                                                            f'{path.join(common.FORTISHIELD_PATH, file)}')
                        update_cluster_control(file, cluster_control_json)
                        continue
                ...
                raise FortishieldError(3001, str(e))
            except Exception as e:
                result_logs['debug'][file].append(f"Exception raised: " + str(FortishieldException(3001, str(e))))
...
def clean_up(node_name=""):
    ...
    try:
        rm_path = path.join(common.FORTISHIELD_PATH, 'queue', 'cluster', node_name)
...
def merge_info(merge_type, node_name, files=None, file_type=""):
    ...
    """
    merge_path = path.join(common.FORTISHIELD_PATH, 'queue', merge_type)
    ...
    with open(path.join(common.FORTISHIELD_PATH, output_file), 'wb') as o_f:
