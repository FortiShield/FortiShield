...
def get_fortishield_active_logging_format() -> LoggingFormat:
    ...
    -------
    LoggingFormat
        Fortishield active log format. Can either be `plain` or `json`. If it has both types, `plain` will be returned.
    """
...
def validate_ossec_conf() -> str:
    ...
    # Message for checking Fortishield configuration
    wcom_msg = common.CHECK_CONFIG_COMMAND

    # Connect to wcom socket
    if exists(wcom_socket_path):
        try:
            wcom_socket = FortishieldSocket(wcom_socket_path)
        except FortishieldException as e:
            extra_msg = f'Socket: FORTISHIELD_PATH/queue/sockets/com. Error {e.message}'
            raise FortishieldInternalError(1013, extra_message=extra_msg)
    else:
    ...
    try:
        ...
        raise FortishieldInternalError(1904, extra_message=str(e))
...
def parse_execd_output(output: str) -> Dict:
    ...
    if error_flag != 0:
        ...
        raise FortishieldError(1908, extra_message=', '.join(errors))
    else:
        response = {'status': 'OK'}
...
async def query_update_check_service(installation_uid: str) -> dict:
    ...
    installation_uid : str
        Fortishield UID to include in the query.
