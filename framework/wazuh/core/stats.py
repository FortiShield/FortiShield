...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise FortishieldError(1308, extra_message=stat_filename)
        ...
        if len(data) == 4:
            ...
            if len(data) != 5:
                if len(data) in (0, 1):
                    ...
                    raise FortishieldInternalError(1309)
...
async def get_daemons_stats_socket(socket: str,
    ...
    """Send message to Fortishield socket to get statistical information.
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise FortishieldInternalError(1104, extra_message=str(e))
    except IOError:
        raise FortishieldError(1308, extra_message=filename)
...
def get_stats_socket_path(agent_id: Union[str, int], daemon: str) -> str:
    ...
    if is_agent_a_manager(agent_id):
        return os.path.join(common.FORTISHIELD_PATH, "queue", "sockets", daemon)
    else:
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise FortishieldError(1117, extra_message=rec_msg)
