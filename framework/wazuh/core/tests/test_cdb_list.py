...
def test_split_key_value_with_quotes(line, expected_key, expected_value):
    ...
    if expected_key and expected_value:
        ...
        with pytest.raises(FortishieldError) as e:
...
def test_get_list_from_file(raw):
    ...
    """
    full_path = os.path.join(common.FORTISHIELD_PATH, PATH_FILE)
    if raw:
...
def test_validate_cdb_list_ko():
    ...
    with pytest.raises(FortishieldError, match=r'\b1112\b'):
        validate_cdb_list('')

    # Raise exeption when using invalid CDB list
    with pytest.raises(FortishieldError, match=r'\b1800\b'):
...
def test_create_list_file_ko():
    ...
    with pytest.raises(FortishieldError, match=r'\b1806\b'):
