...
def list_handler(result: AffectedItemsFortishieldResult, original: dict = None, allowed: dict = None, target: dict = None,
                 add_denied: bool = False, **post_proc_kwargs: dict) -> AffectedItemsFortishieldResult:
        ...
        for res_id, target_param in target.items():
            ...
            for denied_item in denied:
                result.add_failed_item(id_=denied_item,
                                       error=FortishieldPermissionError(4000, extra_message=f'Resource type: {res_id}',
                                                                  ids=denied))
    if not add_denied or post_proc_kwargs.get('force'):
        ...
        if 'default_result_kwargs' in post_proc_kwargs and result is None:
            return AffectedItemsFortishieldResult(**post_proc_kwargs['default_result_kwargs'])
...
def expose_resources(actions: list = None, resources: list = None, post_proc_func: callable = list_handler,
    ...
    """Decorator to apply user permissions on a Fortishield framework function based on exposed action:resource pairs.
            ...
            for res_id, target_param in target_params.items():
                try:
                    ...
                    if add_denied:
                        ...
                        if res_id in integer_resources:
                            denied = {int(i) if i.isdigit() else i for i in denied}
                        raise FortishieldPermissionError(4000,
