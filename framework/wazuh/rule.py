...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsFortishieldResult(none_msg='No rule was returned',
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=FortishieldError(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
    ...
    Raises
    ------
    FortishieldError(1200)
        Error reading rules from `FORTISHIELD_HOME/etc/ossec.conf`.
    ...
    AffectedItemsFortishieldResult
        Affected items.
    """
    result = AffectedItemsFortishieldResult(none_msg='No rule files were returned',
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsFortishieldResult:
    ...
    AffectedItemsFortishieldResult
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsFortishieldResult(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=FortishieldError(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.FORTISHIELD_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.FORTISHIELD_PATH, rules[0]['relative_dirname'], filename))
...
def get_rule_file(filename: str = None, raw: bool = False,
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=FortishieldError(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=FortishieldError(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=FortishieldError(1414, extra_message=f"{filename}"))
...
def validate_upload_delete_dir(relative_dirname: Union[str, None]) -> Tuple[str, FortishieldError]:
    ...
    Tuple (str, FortishieldError)
        ...
        The second element of the tuple is a FortishieldError exception
            If relative_dirname has no 'rule_dir' tag in ruleset return FortishieldError(1505).
            If relative_dirname is inside the default RULES_PATH return FortishieldError(1506).
            If relative_dirname has a 'rule_dir' tag in ruleset but it doesn't exists return FortishieldError(1507).
...
def upload_rule_file(filename: str, content: str, relative_dirname: str = None, 
    ...
    AffectedItemsFortishieldResult
        Affected items.
    """
    result = AffectedItemsFortishieldResult(all_msg='Rule was successfully uploaded',
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsFortishieldResult:
    """Delete a rule file.
    ...
    result = AffectedItemsFortishieldResult(all_msg='Rule was successfully deleted',
