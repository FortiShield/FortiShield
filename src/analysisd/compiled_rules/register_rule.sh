...
if [ "x$1" = "xlist" ]; then
    ...
    if [ "X${2}" = "X" ]; then
        echo "ERROR: You must specify the installation path. i.e.: ${0} $1 FORTISHIELD_HOME"
        exit 1;
    fi    
    FORTISHIELD_HOME=${2}
    ...
    if [ "$FORTISHIELD_TYPE" = "agent" ]; then
        echo "ERROR: You must execute this script on Fortishield Manager"
        exit 1;
    fi

    ls ${FORTISHIELD_HOME}/compiled_rules > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        mkdir ${FORTISHIELD_HOME}/compiled_rules > /dev/null 2>&1
        if [ ! $? = 0 ]; then
    ...
    cp .function_list ${FORTISHIELD_HOME}/compiled_rules/function_list > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        echo "ERROR: Unable to save rules. You must be root to do so."
        exit 1;
    fi
    ...
    for i in `ls *.c`; do
        if [ ! "x$i" = "xgeneric_samples.c" ]; then
            cp $i ${FORTISHIELD_HOME}/compiled_rules/ > /dev/null 2>&1
        fi
    done
    echo "*Save completed at ${FORTISHIELD_HOME}/compiled_rules/";
    exit 0;
    ...
    ls ${FORTISHIELD_HOME}/compiled_rules/function_list > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        echo "*No local compiled rules available to restore."
        exit 0;
    fi

    cat  ${FORTISHIELD_HOME}/compiled_rules/function_list >> .function_list
    if [ ! $? = 0 ]; then
        echo "ERROR: Unable to restore rules. Function list not present."
        exit 1;
    fi

    for i in `ls ${FORTISHIELD_HOME}/compiled_rules/*.c`; do
        if [ ! "x$i" = "xgeneric_samples.c" ]; then
            cp $i ./ > /dev/null 2>&1
        fi
    done
    echo "*Restore completed from ${FORTISHIELD_HOME}/compiled_rules/";
    exit 0;
