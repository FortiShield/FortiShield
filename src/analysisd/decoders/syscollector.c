...
int DecodeSyscollector(Eventinfo *lf,int *socket)
{
    ...
    if (strcmp(msg_type, "port") == 0 || strcmp(msg_type, "port_end") == 0) {
        if (decode_port(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send ports information to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_package(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send packages information to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_hotfix(lf, logJSON, socket) < 0) {
            mdebug1("Unable to send hotfixes information to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_hardware(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send hardware information to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_osinfo(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send osinfo message to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_netinfo(lf, logJSON, socket) < 0) {
            merror("Unable to send netinfo message to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
        ...
        if (decode_process(lf, logJSON,socket) < 0) {
            mdebug1("Unable to send processes information to Fortishield DB.");
            cJSON_Delete (logJSON);
            return (0);
        }
    }
