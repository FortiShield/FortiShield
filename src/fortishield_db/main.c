...
int main(int argc, char ** argv)
{
    ...
    wconfig.worker_pool_size = getDefine_Int("fortishield_db", "worker_pool_size", 1, 32);
    wconfig.commit_time_min = getDefine_Int("fortishield_db", "commit_time_min", 1, 3600);
    wconfig.commit_time_max = getDefine_Int("fortishield_db", "commit_time_max", 1, 3600);
    wconfig.open_db_limit = getDefine_Int("fortishield_db", "open_db_limit", 1, 4096);
    nofile = getDefine_Int("fortishield_db", "rlimit_nofile", 1024, 1048576);

    wconfig.fragmentation_threshold = getDefine_Int("fortishield_db", "fragmentation_threshold", 0, 100);
    wconfig.fragmentation_delta = getDefine_Int("fortishield_db", "fragmentation_delta", 0, 100);
    wconfig.free_pages_percentage = getDefine_Int("fortishield_db", "free_pages_percentage", 0, 99);
    wconfig.max_fragmentation = getDefine_Int("fortishield_db", "max_fragmentation", 0, 100);
    wconfig.check_fragmentation_interval = getDefine_Int("fortishield_db", "check_fragmentation_interval", 1, 30758400);

    // Allocating memory for configuration structures and setting default values
    wdb_init_conf();

    int modules = 0;
    modules |= FORTISHIELDDB;
    modules |= CCLUSTER;

    // Read ossec.conf
    if (ReadConfig(modules, OSSECCONF, &gconfig, NULL) < 0) {
        merror_exit("Invalid configuration block for Fortishield-DB.");
    }

    if (!isDebug()) {
        int debug_level;

        for (debug_level = getDefine_Int("fortishield_db", "debug", 0, 2); debug_level; debug_level--) {
            nowDebug();
        }
    }

    mdebug1(FORTISHIELD_HOMEDIR, home_path);
    ...
    if (!open_dbs) merror_exit("fortishield_db: OSHash_Create() failed");
