...
/* Look for a configuration assessment entry in Fortishield DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Look for a scan configuration assessment entry in Fortishield DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Look for a scan policy entry in Fortishield DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
int wdb_sca_policy_find(wdb_t * wdb, char *id, char * output);

/* Gets the result of all checks in Fortishield DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Gets the result of all policies in Fortishield DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
 ...
 *
 * @param [in] input The chunk obtained from FortishieldDB to be parsed.
 ...
 * a message to process in fortishield-db process.
 *
 * @param wdb The Global struct database.
 * @param input Buffer input
 * @param output Buffer output, on success responses are:
...
/**
 * @brief Function to get the information of a particular agent stored in Fortishield DB.
 *
 * @param wdb The Global struct database.
 * @param id Agent id.
...
/**
 * @brief Method to read the internal fortishield-db configuration.
 *
