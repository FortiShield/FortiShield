/* fortishielddb state management functions
 * May 27, 2022
 *
...
void w_inc_fortishielddb() {
    w_mutex_lock(&db_state_t_mutex);
    wdb_state.queries_breakdown.fortishielddb_queries++;
    w_mutex_unlock(&db_state_t_mutex);
}

void w_inc_fortishielddb_remove() {
    w_mutex_lock(&db_state_t_mutex);
    wdb_state.queries_breakdown.fortishielddb_breakdown.remove_queries++;
    w_mutex_unlock(&db_state_t_mutex);
}

void w_inc_fortishielddb_remove_time(struct timeval time) {
    w_mutex_lock(&db_state_t_mutex);
    timeradd(&wdb_state.queries_breakdown.fortishielddb_breakdown.remove_time, &time, &wdb_state.queries_breakdown.fortishielddb_breakdown.remove_time);
    w_mutex_unlock(&db_state_t_mutex);
}
...
cJSON* wdb_create_state_json() {
    ...
    cJSON_AddNumberToObject(_received_breakdown, "fortishielddb", wdb_state_cpy.queries_breakdown.fortishielddb_queries);

    cJSON *_fortishielddb_breakdown = cJSON_CreateObject();
    cJSON_AddItemToObject(_received_breakdown, "fortishielddb_breakdown", _fortishielddb_breakdown);

    cJSON *_fortishielddb_db = cJSON_CreateObject();
    cJSON_AddItemToObject(_fortishielddb_breakdown, "db", _fortishielddb_db);

    cJSON_AddNumberToObject(_fortishielddb_db, "remove", wdb_state_cpy.queries_breakdown.fortishielddb_breakdown.remove_queries);
    ...
    cJSON_AddNumberToObject(_execution_breakdown, "fortishielddb", timeval_to_milis(wdb_state_cpy.queries_breakdown.fortishielddb_breakdown.remove_time));

    cJSON *_fortishielddb_breakdown_t = cJSON_CreateObject();
    cJSON_AddItemToObject(_execution_breakdown, "fortishielddb_breakdown", _fortishielddb_breakdown_t);

    cJSON *_fortishielddb_db_t = cJSON_CreateObject();
    cJSON_AddItemToObject(_fortishielddb_breakdown_t, "db", _fortishielddb_db_t);

    cJSON_AddNumberToObject(_fortishielddb_db_t, "remove", timeval_to_milis(wdb_state_cpy.queries_breakdown.fortishielddb_breakdown.remove_time));
...
STATIC uint64_t get_time_total(wdb_state_t *state){
    return get_task_time(state) + get_global_time(state) + get_agent_time(state) + timeval_to_milis(state->queries_breakdown.fortishielddb_breakdown.remove_time) + timeval_to_milis(state->queries_breakdown.mitre_breakdown.sql_time);
}
