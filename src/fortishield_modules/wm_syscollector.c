...
static void wm_sys_send_message(const void* data, const char queue_id) {
    ...
    #ifdef CLIENT
            mterror(WM_SYS_LOGTAG, "Unable to send message to '%s' (fortishield-agentd might be down). Attempting to reconnect.", DEFAULTQUEUE);
    #else
            mterror(WM_SYS_LOGTAG, "Unable to send message to '%s' (fortishield-analysisd might be down). Attempting to reconnect.", DEFAULTQUEUE);
    #endif
