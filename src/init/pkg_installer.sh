...
function create_ossec_ug {
    ...
    if [ -z "${exists_user}" ]; then
            ...
            if command -v adduser >/dev/null 2>&1 && command -v dpkg >/dev/null 2>&1; then
                adduser --system --home "${FORTISHIELD_HOME}" --shell ${NO_SHELL} --ingroup ossec ossec >> ./logs/upgrade.log 2>&1
            else
                useradd -g ossec -G ossec -d "${FORTISHIELD_HOME}" -r -s ${NO_SHELL} ossec >> ./logs/upgrade.log 2>&1
            fi
    fi
...
function restore_selinux_policy {
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ $(getenforce) != "Disabled" ]; then
            ...
            echo "$(date +"%Y/%m/%d %H:%M:%S") - Fortishield SELinux module installation skipped (SELinux is disabled)." >> ./logs/upgrade.log
        fi
    fi
}
...
FOLDERS_TO_BACKUP=($FORTISHIELD_HOME/{active-response,bin,etc,lib,queue,ruleset,wodles,agentless,logs/{ossec,wazuh},var/selinux} \
                   ...
                   $SYSTEMD_SERVICE_UNIT_PATH \
                   $INIT_PATH)
FOLDERS_TO_EXCLUDE=($FORTISHIELD_HOME/queue/diff)
...
if [ $RESULT -eq 0 ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Backup generated in ${FORTISHIELD_HOME}/backup/backup_[${BDATE}].tar.gz" >> ./logs/upgrade.log
else
...
if [ "${FORTISHIELD_REVISION}" -lt "40300" ]; then
    RESTORE_OSSEC_OWN=1
