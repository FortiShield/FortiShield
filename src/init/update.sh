...
# Shell script update functions for Fortishield
...
##########
# Checks if Fortishield is installed by taking the installdir from the services
...
getPreinstalledDirByType()
{
    ...
    if [ -r "/etc/redhat-release" ]; then
        if [ -d /etc/rc.d/init.d ]; then
            if [ -f /etc/rc.d/init.d/${pidir_service_name} ]; then
                PREINSTALLEDDIR=`sed -n 's/^FORTISHIELD_HOME=\(.*\)$/\1/p' /etc/rc.d/init.d/${pidir_service_name}`
                if [ -d "$PREINSTALLEDDIR" ]; then
                    return 0;
                else
                    return 1;
    ...
    if [ -r "/etc/gentoo-release" ]; then
        if [ -f /etc/init.d/${pidir_service_name} ]; then
            PREINSTALLEDDIR=`sed -n 's/^FORTISHIELD_HOME=\(.*\)$/\1/p' /etc/init.d/${pidir_service_name}`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ -r "/etc/slackware-version" ]; then
        if [ -f /etc/rc.d/rc.${pidir_service_name} ]; then
            PREINSTALLEDDIR=`sed -n 's/^FORTISHIELD_HOME=\(.*\)$/\1/p' /etc/rc.d/rc.${pidir_service_name}`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
    ...
    if [ "X${UN}" = "XHP-UX" ]; then
        if [ -f /sbin/init.d/${pidir_service_name} ]; then
            PREINSTALLEDDIR=`sed -n 's/^FORTISHIELD_HOME=\(.*\)$/\1/p' /sbin/init.d/${pidir_service_name}`
            if [ -d "$PREINSTALLEDDIR" ]; then
                return 0;
            else
                return 1;
...
##########
# Checks if Fortishield is installed in the specified path by searching for the control binary.
#
...
##########
# Checks if Fortishield is installed by trying with each installation type.
# If it finds an installation, it sets the PREINSTALLEDDIR variable.
# After that it checks if Fortishield is truly installed there, if it is installed it returns TRUE.
# If it isn't installed continue searching in other installation types and replacing PREINSTALLEDDIR variable.
# It returns FALSE if Fortishield isn't installed in any of this.
#
# getPreinstalledDir()
##########
getPreinstalledDir()
{
    ...
    if [ -f "${OSSEC_INIT}" ]; then
        ...
        if [ -d "$DIRECTORY" ]; then
            ...
            if isFortishieldInstalled $PREINSTALLEDDIR; then
                return 0;
            fi
        fi
    fi
...
UpdateStopOSSEC()
{
   ...
   # We also need to remove all syscheck queue file (format changed)
    ...
    # Deleting plain-text agent information if exists (it was migrated to Fortishield DB in v4.1)
    if [ -d "$PREINSTALLEDDIR/queue/agent-info" ]; then
        rm -rf $PREINSTALLEDDIR/queue/agent-info > /dev/null 2>&1
    fi

    # Deleting plain-text rootcheck information if exists (it was migrated to Fortishield DB in v4.1)
...
UpdateOldVersions()
{
    ...
    # If it is Fortishield 2.0 or newer, exit
    ...
    if [ ! "$INSTYPE" = "agent" ]; then
        ...
        # Backup decoders: Fortishield v1.0 and OSSEC
