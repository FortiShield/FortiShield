...
/**
 * @brief Connects to Fortishield-DB socket
 *
...
/**
 * @brief Sends query to Fortishield-DB and stores the response.
 *
 * @param[in] sock Client socket descriptor.
 * @param[in] query Query to be sent to Fortishield-DB.
 * @param[out] response Char pointer where the response from Fortishield-DB will be stored.
 * @param[in] len Lenght of the response param.
 * @post This function will read up to len bytes from Fortishield DB.
...
/**
 * @brief Check connection to Fortishield-DB, sends query and stores the response.
 *
...
int wdbc_query_ex(int *sock, const char *query, char *response, const int len) {
    ...
    if (retval = wdbc_query(*sock, query, response, len), retval != 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            // Retry to connect
            merror("Connection with fortishield-db lost. Reconnecting.");
            close(*sock);
...
/**
 * @brief Parse the result of the query to Fortishield-DB
 *
 ...
 *
 * @param result Result from the query to Fortishield-DB.
...
cJSON * wdbc_query_parse_json(int *sock, const char *query, char *response, const int len) {
    ...
    case -2:
        ...
        return NULL;
    case -1:
        merror("No response from fortishield-db.");
        return NULL;
    }
    ...
    case WDBC_OK:
        break;
    case WDBC_ERROR:
        merror("Bad response from fortishield-db: %s", arg);
        // Fallthrough
    default:
        return NULL;
    }
 ...
 * @param[out] response Char pointer where the response from Fortishield-DB will be stored.
 * @param[in] len Lenght of the response param.
 * @param[out] payload Char pointer where the payload from Fortishield-DB will be stored.
 * @return Enum wdbc_result.
 */
...
wdbc_result wdbc_query_parse(int *sock, const char *query, char *response, const int len, char** payload) {
    ...
    if (OS_SUCCESS == result) {
        ...
        if (status == WDBC_ERROR){
            merror("Bad response from fortishield-db: %s", _payload);
        }
    }
    else if (-2 == result) {
        merror("Unable to connect to socket '%s'", WDB_LOCAL_SOCK);
