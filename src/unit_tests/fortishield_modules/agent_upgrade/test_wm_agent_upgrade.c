...
void test_wm_agent_upgrade_dump_enabled(void **state)
{
    ...
    #ifdef TEST_SERVER
    os_strdup("fortishield.github.io/packages", config->manager_config.wpk_repository);
    ...
    assert_string_equal(cJSON_GetObjectItem(conf, "wpk_repository")->valuestring, "fortishield.github.io/packages");
    #else
