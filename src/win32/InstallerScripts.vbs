...
' Copyright (C) 2015, Fortishield Inc. <support@fortishield.github.io>
'
...
public function config()
    ...
    FORTISHIELD_MANAGER = Replace(args(2), Chr(34), "")
    FORTISHIELD_MANAGER_PORT = Replace(args(3), Chr(34), "")
    FORTISHIELD_PROTOCOL = Replace(args(4), Chr(34), "")
    NOTIFY_TIME = Replace(args(5), Chr(34), "")
    FORTISHIELD_REGISTRATION_SERVER = Replace(args(6), Chr(34), "")
    FORTISHIELD_REGISTRATION_PORT = Replace(args(7), Chr(34), "")
    FORTISHIELD_REGISTRATION_PASSWORD = Replace(args(8), Chr(34), "")
    FORTISHIELD_KEEP_ALIVE_INTERVAL = Replace(args(9), Chr(34), "")
    FORTISHIELD_TIME_RECONNECT = Replace(args(10), Chr(34), "")
    FORTISHIELD_REGISTRATION_CA = Replace(args(11), Chr(34), "")
    FORTISHIELD_REGISTRATION_CERTIFICATE = Replace(args(12), Chr(34), "")
    FORTISHIELD_REGISTRATION_KEY = Replace(args(13), Chr(34), "")
    FORTISHIELD_AGENT_NAME = Replace(args(14), Chr(34), "")
    FORTISHIELD_AGENT_GROUP = Replace(args(15), Chr(34), "")
    ...
    If objFSO.fileExists(home_dir & "ossec.conf") Then
        ...
        If FORTISHIELD_MANAGER <> "" or FORTISHIELD_MANAGER_PORT <> "" or FORTISHIELD_PROTOCOL <> "" or FORTISHIELD_KEEP_ALIVE_INTERVAL <> "" or FORTISHIELD_TIME_RECONNECT <> "" Then
            If FORTISHIELD_PROTOCOL <> "" and InStr(FORTISHIELD_PROTOCOL,",") Then
                protocol_list=Split(LCase(FORTISHIELD_PROTOCOL),",")
            Else
                protocol_list=Array(LCase(FORTISHIELD_PROTOCOL))
            End If
            If FORTISHIELD_MANAGER <> "" Then 
                Set re = new regexp
                re.Pattern = "\s+<server>(.|\n)+?</server>"
                If InStr(FORTISHIELD_MANAGER,",") Then
                    ip_list=Split(FORTISHIELD_MANAGER,",")
                Else
                    ip_list=Array(FORTISHIELD_MANAGER)
                End If

                unique_protocol_list=get_unique_array_values(protocol_list)

                if ( UBound(protocol_list) >= UBound(ip_list) And UBound(unique_protocol_list) = 0 ) Or (FORTISHIELD_PROTOCOL = "") Or ( UBound(unique_protocol_list) = 0 And LCase(unique_protocol_list(0)) = "tcp" ) Then
            ...
            If FORTISHIELD_MANAGER_PORT <> "" Then ' manager server_port
                If InStr(strText, "<port>") > 0 Then
                    strText = Replace(strText, "<port>1514</port>", "<port>" & FORTISHIELD_MANAGER_PORT & "</port>")
                End If

            End If

            If FORTISHIELD_KEEP_ALIVE_INTERVAL <> "" Then
                If InStr(strText, "<notify_time>") > 0 Then
                    Set re = new regexp
                    re.Pattern = "<notify_time>.*</notify_time>"
                    re.Global = True
                    strText = re.Replace(strText, "<notify_time>" & FORTISHIELD_KEEP_ALIVE_INTERVAL & "</notify_time>")
                End If
            End If

            If FORTISHIELD_TIME_RECONNECT <> "" Then 'TODO fix the - and use _
                If InStr(strText, "<time-reconnect>") > 0 Then
                    Set re = new regexp
                    re.Pattern = "<time-reconnect>.*</time-reconnect>"
                    re.Global = True
                    strText = re.Replace(strText, "<time-reconnect>" & FORTISHIELD_TIME_RECONNECT & "</time-reconnect>")
                End If
            End If
        End If
        
        If FORTISHIELD_REGISTRATION_SERVER <> "" or FORTISHIELD_REGISTRATION_PORT <> "" or FORTISHIELD_REGISTRATION_PASSWORD <> "" or FORTISHIELD_REGISTRATION_CA <> "" or FORTISHIELD_REGISTRATION_CERTIFICATE <> "" or FORTISHIELD_REGISTRATION_KEY <> "" or FORTISHIELD_AGENT_NAME <> "" or FORTISHIELD_AGENT_GROUP <> "" or ENROLLMENT_DELAY <> "" Then
            ...
            If FORTISHIELD_REGISTRATION_SERVER <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <manager_address>" & FORTISHIELD_REGISTRATION_SERVER & "</manager_address>"& vbCrLf &"    </enrollment>")
            End If  
            
            If FORTISHIELD_REGISTRATION_PORT <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <port>" & FORTISHIELD_REGISTRATION_PORT & "</port>"& vbCrLf &"    </enrollment>")
            End If
            
            If FORTISHIELD_REGISTRATION_PASSWORD <> "" Then
                Set objFile = objFSO.CreateTextFile(home_dir & "authd.pass", ForWriting)
                objFile.WriteLine FORTISHIELD_REGISTRATION_PASSWORD
                objFile.Close
                strText = Replace(strText, "    </enrollment>", "      <authorization_pass_path>authd.pass</authorization_pass_path>"& vbCrLf &"    </enrollment>")
            End If

            If FORTISHIELD_REGISTRATION_CA <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <server_ca_path>" & FORTISHIELD_REGISTRATION_CA & "</server_ca_path>"& vbCrLf &"    </enrollment>")
            End If

            If FORTISHIELD_REGISTRATION_CERTIFICATE <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_certificate_path>" & FORTISHIELD_REGISTRATION_CERTIFICATE & "</agent_certificate_path>"& vbCrLf &"    </enrollment>")
            End If

            If FORTISHIELD_REGISTRATION_KEY <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_key_path>" & FORTISHIELD_REGISTRATION_KEY & "</agent_key_path>"& vbCrLf &"    </enrollment>")
            End If

            If FORTISHIELD_AGENT_NAME <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_name>" & FORTISHIELD_AGENT_NAME & "</agent_name>"& vbCrLf &"    </enrollment>")
            End If

            If FORTISHIELD_AGENT_GROUP <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <groups>" & FORTISHIELD_AGENT_GROUP & "</groups>"& vbCrLf &"    </enrollment>")
            End If
...
Public Function CheckSvcRunning()
    ...
    SERVICE = "FortishieldSvc"
    Set svc = wmi.ExecQuery("Select * from Win32_Service where Name = '" & SERVICE & "'")

    If svc.Count <> 0 Then
        state = wmi.Get("Win32_Service.Name='" & SERVICE & "'").State
        Session.Property("FORTISHIELDRUNNING") = state
    End If
...
Public Function StartFortishieldSvc()
	Set WshShell = CreateObject("WScript.Shell")
    StartSvc = "NET START FortishieldSvc"
