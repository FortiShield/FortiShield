...
def populate_dict(dict, files_list):
    for file in files_list:
        ...
        if file.name.count('FORTISHIELD_AGENT.EXE') > 0:
            continue
        ...
        dict[file.name.lower().replace('-', '_').replace('c++', 'cpp').replace(
            '_dll', '.dll').replace('fortishield_agent_eventchannel.exe', 'fortishield_agent.exe').replace('libfimdb.dll', 'fimdb.dll')] = file_hash
