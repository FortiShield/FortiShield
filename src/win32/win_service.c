...
static LPTSTR g_lpszServiceName        = "FortishieldSvc";
static LPTSTR g_lpszServiceDisplayName = "Fortishield";
static LPTSTR g_lpszServiceDescription = "Fortishield Windows Agent";
...
int os_stop_service()
{
    ...
    * newer instances of fortishield-agent.
    */
    ...
    if (ossecServiceStatusHandle) {
        switch (dwOpcode) {
            case SERVICE_CONTROL_STOP:
                ...
                // Kill children processes spawned by modules, only in fortishield-agent
                wm_kill_children();
                stop_wmodules();
