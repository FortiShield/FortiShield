...
def truncate_monitored_files_implementation() -> None:
    ...
    if services.get_service() == FORTISHIELD_MANAGER:
        log_files = [FORTISHIELD_LOG_PATH, ALERTS_JSON_PATH, FORTISHIELD_API_LOG_FILE_PATH,
                     FORTISHIELD_API_JSON_LOG_FILE_PATH, FORTISHIELD_CLIENT_KEYS_PATH]
    else:
        log_files = [FORTISHIELD_LOG_PATH]
...
def restart_wazuh_daemon_after_finishing_module(daemon: str = None) -> None:
    ...
    """
    yield
    file.truncate_file(FORTISHIELD_LOG_PATH)
    services.control_service("restart", daemon=daemon)
...
def configure_local_internal_options_handler(request: pytest.FixtureRequest, test_metadata) -> None:
    ...
    The `local_internal_options` is a dict with keys and values as the Fortishield `local_internal_options` format.
...
def autostart_simulators(request: pytest.FixtureRequest) -> None:
    ...
    if services.get_service() is not FORTISHIELD_MANAGER:
