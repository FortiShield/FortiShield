...
from fortishield_testing.modules.agentd.patterns import AGENTD_REQUESTING_KEY
...
def test_agentd_initial_enrollment_retries(test_metadata, set_fortishield_configuration, configure_local_internal_options, truncate_monitored_files, remove_keys_file, daemons_handler):
    '''
    ...
    fortishield_log_monitor.start(callback=callbacks.generate_callback(AGENTD_REQUESTING_KEY,{'IP':''}), timeout = 300, accumulations = 4)
    assert (fortishield_log_monitor.callback_result != None), f'Enrollment retries was not sent'
