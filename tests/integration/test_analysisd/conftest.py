...
def configure_local_rules(request, test_configuration):
    """Configure a custom rule for testing. Restart Fortishield is needed for applying the configuration. """
...
def configure_remove_tags(request, test_metadata):
    """Configure a custom settting for testing. Restart Fortishield is needed for applying the configuration. """
    # Remove test case tags from ossec.conf
    file.replace_regex_in_file(test_metadata['remove_tags'], [''] * len(test_metadata['remove_tags']), FORTISHIELD_CONF_PATH)
