...
def add_configuration(test_configuration: list[dict], request: pytest.FixtureRequest) -> None:
    """Add configuration to the Fortishield API configuration files.
...
def wait_for_api_start(test_configuration: dict) -> None:
    ...
    if test_configuration is not None:
        if test_configuration.get('blocks') is not None:
            ...
            port = test_configuration['blocks'].get('port', FORTISHIELD_API_PORT)

    file_to_monitor = FORTISHIELD_API_JSON_LOG_FILE_PATH if logs_format == 'json' else FORTISHIELD_API_LOG_FILE_PATH
