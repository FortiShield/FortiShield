...
```
openssl req -new -nodes -newkey rsa:2048 -keyout wpkcert.key -out wpkcert.csr -subj '/C=US/ST=CA/O=Fortishield'
```

- `/C=US` is the country.
- `/ST=CA` is the state.
- `/O=Fortishield` is the organization's name.
...
In this particular case, the Fortishield Project's root directory contains the proper `upgrade.sh` file.
...
1. Overwrite the shipped root CA with your certificate. This will prevent your agent from upgrading using WPK packages from Fortishield.
...
Get the WPK package into the Fortishield manager and run:
