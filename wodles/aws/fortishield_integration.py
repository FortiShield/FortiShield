...
MESSAGE_HEADER = "1:Fortishield-AWS:"


class FortishieldIntegration:
    """
    ...
    def default_config(profile: str) -> dict:
        ...
        This method is called when Fortishield Integration is instantiated and sets a default config using .aws/config file
        ...
        """
        args = {}
        args['config'] = botocore.config.Config(retries=aws_tools.FORTISHIELD_DEFAULT_RETRY_CONFIGURATION)
    ...
    def send_msg(self, msg, dump_json=True):
        """
        Sends an AWS event to the Fortishield Queue
        ...
        try:
            ...
            if e.errno == 111:
                print("ERROR: Fortishield must be running.")
                sys.exit(11)
            elif e.errno == 90:
                print("ERROR: Message too long to send to Fortishield.  Skipping message...")
...
class FortishieldAWSDatabase(FortishieldIntegration):
    """
    ...
    def __init__(self, profile, iam_role_arn, db_name,
        ...
        FortishieldIntegration.__init__(self, service_name=service_name,
                                  profile=profile,
